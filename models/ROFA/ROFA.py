...
from models.ROFA.base_block import *
import torch.nn.functional as F
import warnings
...
class CFFusion(nn.Module):
    def __init__(self, dim, height=2, reduction=8):
        super(CFFusion, self).__init__()
...
class OFF_Decoder(nn.Module):
    ...
    def __init__(self, embed_dim=256):
        ...
        self.dr1 = nn.Conv2d(embed_dim, 1, kernel_size=1, padding=0)
        self.dr2 = nn.Conv2d(embed_dim, 1, kernel_size=1, padding=0)
...
class ROFA(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.backbone = Backbone(patch_size=7, in_chans=input_nc, num_classes=output_nc, embed_dims=self.embed_dims,
                                 num_heads=[1, 4, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None,
                                 drop_rate=self.drop_rate,
                                 attn_drop_rate=self.attn_drop, drop_path_rate=self.drop_path_rate,
                                 norm_layer=partial(nn.LayerNorm, eps=1e-6),
                                 depths=self.depths, sr_ratios=[4, 2, 2, 1])
        ...
        self.OFF_Decoder_1 = OFF_Decoder(embed_dim=512)
        self.OFF_Decoder_2 = OFF_Decoder(embed_dim=256)
        self.OFF_Decoder_3 = OFF_Decoder(embed_dim=128)
        self.sigmoid = nn.Sigmoid()

        self.conv_rd = nn.ModuleList(
            [
                ConvReduce(2 * ch, ch)
    ...
    def forward(self, x1, x2):
        ...
        off = self.OFF_Decoder_1(fx1[len(fx1) - 1], fx2[len(fx1) - 1])

        off_d1 = self.OFF_Decoder_2(fx1[len(fx1) - 2], fx2[len(fx1) - 2])
        off_d2 = self.OFF_Decoder_3(fx1[len(fx1) - 3], fx2[len(fx1) - 3])

        off_all = off + off_d1 + off_d2

        A_aligned = self.apt(x1, off_all)
        fx1_1 = self.backbone(A_aligned)

        for i in range(len(fx1)):
            fx1_aligned.append(self.apt(fx1[i], off_all / (4 * (i + 1))))

        fx1_new = []
        for i in range(len(fx1)):
            fx1_new.append(self.fusion[i]([fx1_1[i], fx1_aligned[i]]))

        cp = self.CD_Decoder(fx1_new, fx2)
        for i in range(len(cp)):
            cp[i] = resize(cp[i], size=x1.size()[2:], mode='bilinear', align_corners=False)

        return cp, off, off_d1, off_d2, A_aligned